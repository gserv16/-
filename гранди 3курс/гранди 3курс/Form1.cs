...
namespace Гранди
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            InitializeComponent();
            random = new Random();
            firstDivision.TextChanged += firstCountOfItems_TextChanged;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (!int.TryParse(initialCount.Text, out int initialHeap) || initialHeap <= 2)
            {
                ...
                return;
                DialogResult result;
                if (initialCount != null) result = MessageBox.Show("Заполните поля для начала игры", "Начало игры",
                    MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button2);
            }

            heaps = new List<int> { initialHeap };
            isPlayerTurn = radioButton1.Checked;
            UpdateHeapsDisplay();
        ...
        private void MakePlayerMove()
        {
            ...
            if (selectedHeap <= 2 || newHeap1 + newHeap2 != selectedHeap || newHeap1 == newHeap2 || newHeap1 <= 0 || newHeap2 <= 0)
            {
                MessageBox.Show("Некорректные размеры новых куч.");
                return;
            }
        ...
        private void firstCountOfItems_TextChanged(object sender, EventArgs e)
        {
